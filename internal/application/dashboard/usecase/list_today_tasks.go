...
type ListTodayTasksUseCase struct {
	taskService *service.TaskService
	logger      logger.Logger
}

func NewListTodayTasksUseCase(svc *service.TaskService, l logger.Logger) *ListTodayTasksUseCase {
	return &ListTodayTasksUseCase{
		taskService: svc,
		logger:      l,
	}
}

func (uc *ListTodayTasksUseCase) Execute(ctx context.Context, nodeIDStr string, date *time.Time) (*dashboard.TodayTasksResponse, error) {
	// Validate input
	if nodeIDStr == "" {
	...
	if date != nil {
		today = *date
		uc.logger.Info("Using custom date for today's tasks", map[string]interface{}{
			"date": today.Format("2006-01-02"),
		})
	}

	// Get today's tasks from service
	tasks, err := uc.taskService.ListTodayTasks(ctx, nodeID, today)
	if err != nil {
		uc.logger.Error("Failed to list today's tasks", map[string]interface{}{
	...
	response := &dashboard.TodayTasksResponse{
		Items: items,
		Total: len(items),
	}

	uc.logger.Info("Today's tasks retrieved successfully", map[string]interface{}{
		"nodeID": nodeID.String(),
		"date":   today.Format("2006-01-02"),
		"count":  len(items),
	})
